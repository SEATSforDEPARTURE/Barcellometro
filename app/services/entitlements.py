...
DEFAULT_COMMAND_PROFILE_CONFIG = {
    "allowed": True,
    "output": {
        "show_score": True,
...
class EntitlementsService:
    ...
    async def get_command_profile_config(self, member: Any, command: str) -> dict[str, Any]:
        ...
        raw_config = profiles.get(profile) or profiles.get("base") or {}
        return self._normalize_profile_config(raw_config)
    ...
    def _normalize_profile_config(self, raw_config: Any) -> dict[str, Any]:
        output_defaults = dict(DEFAULT_COMMAND_PROFILE_CONFIG["output"])
        messages_defaults = dict(DEFAULT_COMMAND_PROFILE_CONFIG["messages"])
        config = {
            "allowed": DEFAULT_COMMAND_PROFILE_CONFIG["allowed"],
            "output": dict(output_defaults),
            "capabilities": list(DEFAULT_COMMAND_PROFILE_CONFIG["capabilities"]),
            "messages": dict(messages_defaults),
        }
        if not isinstance(raw_config, dict):
            return config

        allowed = raw_config.get("allowed", config["allowed"])
        config["allowed"] = bool(allowed)

        output = raw_config.get("output", {})
        if isinstance(output, dict):
            merged_output = dict(output_defaults)
            for key in output_defaults:
                if key in output:
                    merged_output[key] = bool(output[key])
            config["output"] = merged_output
        else:
            config["output"] = output_defaults

        capabilities = raw_config.get("capabilities", [])
        if isinstance(capabilities, list):
            config["capabilities"] = [str(item) for item in capabilities]
        else:
            config["capabilities"] = []

        messages = raw_config.get("messages", {})
        if isinstance(messages, dict):
            merged_messages = dict(messages_defaults)
            for key in ("dm_text", "footer_text"):
                if isinstance(messages.get(key), str):
                    merged_messages[key] = messages[key]
            config["messages"] = merged_messages
        else:
            config["messages"] = messages_defaults
