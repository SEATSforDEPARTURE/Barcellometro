    ...
    def _build_decode_error_handler(
        ...
        def _on_decode_error(exc: Exception, context: DecodeErrorContext) -> None:
            ...
            if decode_type == "crypto":
                logger.error(
                    "Voice receive adapter crypto decode failure source=%s root_cause_source=%s event_id=%s error_class=%s error_message=%s user=%s ssrc=%s packet_sequence=%s packet_timestamp=%s payload_len=%s packet_origin=%s decoder_instance_id=%s context_session=%s context_guild=%s context_channel=%s",
                    context.source,
                    context.root_cause_source,
                    context.event_id,
                    context.error_class or type(exc).__name__,
                    context.error_message or str(exc),
                    context.user_id,
                    context.ssrc,
                ...
                logger.warning(
                    "Voice receive adapter opus decode failure source=%s root_cause_source=%s event_id=%s error_class=%s error_message=%s user=%s ssrc=%s packet_sequence=%s packet_timestamp=%s payload_len=%s packet_origin=%s packet_type=%s payload_preview_hex=%s payload_looks_like_rtp=%s decoder_instance_id=%s context_session=%s context_guild=%s context_channel=%s",
                    context.source,
                    context.root_cause_source,
                    context.event_id,
                    context.error_class or type(exc).__name__,
