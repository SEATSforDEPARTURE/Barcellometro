...
from app.services.embed_public_service_keys import list_public_embed_service_keys, resolve_public_embed_service_key

_MAX_AUTOCOMPLETE_CHOICES = 25


async def list_embed_template_services(ctx: object | None = None) -> list[str]:
    del ctx
    return list_public_embed_service_keys()


def resolve_embed_template_public_service(service_name: str | None) -> str | None:
    return resolve_public_embed_service_key(service_name)
...
async def build_embed_template_service_autocomplete_choices(ctx: object | None, current: str) -> list[app_commands.Choice[str]]:
