...
from app.plugins.commands_modular.permissions import canonical_permission_key, check_permission, legacy_permission_candidates
...
def _policy_command_matches(row_command: str, command_filter: str) -> bool:
    candidates = {_canonical_policy_command(command_filter), *legacy_permission_candidates(command_filter)}
    return str(row_command).strip().lower() in candidates
...
def register_roles(commandguard_group: app_commands.Group, ctx: CommandContext) -> None:
    ...
    async def role_add_command(interaction: discord.Interaction, role: discord.Role, command: str, usage_limit: int | None = None, cooldown_seconds: int | None = None) -> None:
        if not await check_permission(interaction, "admin.commandguard.role_add", ctx, legacy_aliases=["bm.role.set_role"]):
            return
    ...
    async def role_edit_command(interaction: discord.Interaction, role: discord.Role, command: str, usage_limit: int | None = None, cooldown_seconds: int | None = None) -> None:
        if not await check_permission(interaction, "admin.commandguard.role_edit", ctx, legacy_aliases=["bm.role.set_role"]):
            return
    ...
    async def role_remove_command(interaction: discord.Interaction, role: discord.Role, command: str) -> None:
        if not await check_permission(interaction, "admin.commandguard.role_remove", ctx, legacy_aliases=["bm.role.clear_role"]):
            return
        for candidate in dict.fromkeys([_canonical_policy_command(command), *legacy_permission_candidates(command)]):
            await ctx.database.delete_role_policy(guild_id=str(interaction.guild_id), role_id=str(role.id), command=candidate)
    ...
    async def role_show_command(interaction: discord.Interaction, role: discord.Role, command: str | None = None) -> None:
        if not await check_permission(interaction, "admin.commandguard.role_show", ctx, legacy_aliases=["bm.role.show_role"]):
            return
    ...
    async def role_list_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "admin.commandguard.role_list", ctx, legacy_aliases=["bm.role.show_role"]):
            return
    ...
    async def role_reset_command(interaction: discord.Interaction, role: discord.Role) -> None:
        if not await check_permission(interaction, "admin.commandguard.role_reset", ctx, legacy_aliases=["bm.role.clear_role"]):
            return
    ...
    async def user_add_command(interaction: discord.Interaction, user: discord.User, command: str, usage_limit: int | None = None, cooldown_seconds: int | None = None) -> None:
        if not await check_permission(interaction, "admin.commandguard.user_add", ctx, legacy_aliases=["bm.role.set_user"]):
            return
    ...
    async def user_edit_command(interaction: discord.Interaction, user: discord.User, command: str, usage_limit: int | None = None, cooldown_seconds: int | None = None) -> None:
        if not await check_permission(interaction, "admin.commandguard.user_edit", ctx, legacy_aliases=["bm.role.set_user"]):
            return
    ...
    async def user_remove_command(interaction: discord.Interaction, user: discord.User, command: str) -> None:
        if not await check_permission(interaction, "admin.commandguard.user_remove", ctx, legacy_aliases=["bm.role.clear_user"]):
            return
        for candidate in dict.fromkeys([_canonical_policy_command(command), *legacy_permission_candidates(command)]):
            await ctx.database.delete_user_policy(guild_id=str(interaction.guild_id), user_id=str(user.id), command=candidate)
    ...
    async def user_show_command(interaction: discord.Interaction, user: discord.User, command: str | None = None) -> None:
        if not await check_permission(interaction, "admin.commandguard.user_show", ctx, legacy_aliases=["bm.role.show_user"]):
            return
    ...
    async def user_list_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "admin.commandguard.user_list", ctx, legacy_aliases=["bm.role.show_user"]):
            return
    ...
    async def user_reset_command(interaction: discord.Interaction, user: discord.User) -> None:
        if not await check_permission(interaction, "admin.commandguard.user_reset", ctx, legacy_aliases=["bm.role.clear_user"]):
            return
