...
def _policy_line(command: str, usage_limit: int | None, cooldown_seconds: int | None) -> str:
    ...
    return f"{command}: limit={limit} cooldown={cooldown}"


async def _check_policy_values(
    interaction: discord.Interaction,
    usage_limit: int | None,
    cooldown_seconds: int | None,
) -> bool:
    if usage_limit is not None and usage_limit <= 0:
        await interaction.response.send_message("Please provide a valid usage limit.", ephemeral=True)
        return False
    if cooldown_seconds is not None and cooldown_seconds < 0:
        await interaction.response.send_message("Please provide a valid cooldown.", ephemeral=True)
        return False
    return True
...
def register_roles(commandguard_group: app_commands.Group, ctx: CommandContext) -> None:
    ...
    @app_commands.describe(
        ...
        usage_limit="Optional daily usage limit.",
    ...
    async def role_add_command(
        ...
        if not await check_permission(interaction, "bm.commandguard.role_add", ctx, legacy_aliases=["bm.role.set_role"]):
            return
        if not await _check_policy_values(interaction, usage_limit, cooldown_seconds):
            return
        ...
        await interaction.response.send_message("Role policy added.", ephemeral=True)
    ...
    async def role_edit_command(
        ...
        await interaction.response.send_message("Role policy updated.", ephemeral=True)
    ...
    async def role_remove_command(
        ...
        await interaction.response.send_message("Role policy removed.", ephemeral=True)
    ...
    async def role_show_command(
        ...
        if not rows:
            await interaction.response.send_message("No policies found for that role.", ephemeral=True)
            return
        lines = [_policy_line(row["command"], row["usage_limit"], row["cooldown_seconds"]) for row in rows]
    ...
    async def role_list_command(interaction: discord.Interaction) -> None:
        ...
        if not rows:
            await interaction.response.send_message("No role policies configured.", ephemeral=True)
            return
        lines = [
            f"role={row['role_id']} · {_policy_line(row['command'], row['usage_limit'], row['cooldown_seconds'])}"
            for row in rows
        ]
        await interaction.response.send_message("\n".join(lines), ephemeral=True)
    ...
    async def role_reset_command(interaction: discord.Interaction, role: discord.Role) -> None:
        ...
        await interaction.response.send_message("All role policies reset.", ephemeral=True)
    ...
    async def user_add_command(
        ...
        await interaction.response.send_message("User policy added.", ephemeral=True)
    ...
    async def user_edit_command(
        ...
        await interaction.response.send_message("User policy updated.", ephemeral=True)
    ...
    async def user_remove_command(
        ...
        await interaction.response.send_message("User policy removed.", ephemeral=True)
    ...
    async def user_show_command(
        ...
        if command is not None:
            rows = [row for row in rows if row["command"] == command]
        if not rows:
            await interaction.response.send_message("No policies found for that user.", ephemeral=True)
            return
        lines = [_policy_line(row["command"], row["usage_limit"], row["cooldown_seconds"]) for row in rows]
        await interaction.response.send_message("\n".join(lines), ephemeral=True)
    ...
    async def user_list_command(interaction: discord.Interaction) -> None:
        ...
        if not rows:
            await interaction.response.send_message("No user policies configured.", ephemeral=True)
            return
        lines = [
            f"user={row['user_id']} · {_policy_line(row['command'], row['usage_limit'], row['cooldown_seconds'])}"
            for row in rows
        ]
        await interaction.response.send_message("\n".join(lines), ephemeral=True)
    ...
    async def user_reset_command(interaction: discord.Interaction, user: discord.User) -> None:
        ...
        await interaction.response.send_message("All user policies reset.", ephemeral=True)
