...
async def _send_ai_map(
    ...
    *,
    ...
    action = "model_show" if "primary" in title.lower() else "fallback_show"
    if task is not None:
        await _send_legacy(
            interaction,
            ctx,
...
async def _require_guild_channel(interaction: discord.Interaction, ctx: CommandContext) -> discord.abc.GuildChannel | None:
...
async def _set_events_enabled(interaction: discord.Interaction, ctx: CommandContext, enabled: bool) -> None:
    channel = await _require_guild_channel(interaction, ctx)
    if channel is None:
        return
...
async def _show_events_status(interaction: discord.Interaction, ctx: CommandContext) -> None:
    channel = await _require_guild_channel(interaction, ctx)
    if channel is None:
        return
...
def register_database(database_group: app_commands.Group, ctx: CommandContext) -> None:
    ...
    database_group.add_command(events_group)
    database_group.add_command(retention_group)
    database_group.add_command(backfill_group)
    ...
    async def events_on_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.events.on", ctx):
            return
        await _set_events_enabled(interaction, ctx, True)
    ...
    async def events_off_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.events.off", ctx):
            return
        await _set_events_enabled(interaction, ctx, False)
    ...
    async def events_status_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.events.status", ctx):
            return
        await _show_events_status(interaction, ctx)
    ...
    async def retention_on_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.retention.on", ctx):
            return
        await ctx.retention.set_enabled(True)
        await _send_legacy(interaction, ctx, top_level="database", path_parts=["retention", "on"], entries=[("Status", "enabled")], tone="success", service_name="status")
    ...
    async def retention_off_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.retention.off", ctx):
            return
        await ctx.retention.set_enabled(False)
        await _send_legacy(interaction, ctx, top_level="database", path_parts=["retention", "off"], entries=[("Status", "disabled")], tone="success", service_name="status")
    ...
    async def retention_status_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.retention.status", ctx):
            return
    ...
    @retention_group.command(name="limits_set", description="Update retention limits.")
    @app_commands.describe(days="Retention window in days.")
    async def retention_limits_set_command(interaction: discord.Interaction, days: int | None = None) -> None:
        if not await check_permission(interaction, "database.retention.limits_set", ctx):
            return
        if days is None:
            await _send_legacy(
                interaction,
                ctx,
                top_level="database",
                path_parts=["retention", "limits_set"],
                entries=[("Reason", "No changes provided")],
                tone="warning",
                sections=[("Next Step", [("Command", "/database retention limits_show")])],
                service_name="status",
            )
            return
        if days <= 0:
    ...
    @retention_group.command(name="limits_show", description="Show retention limits.")
    async def retention_limits_show_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.retention.limits_show", ctx):
            return
        await _send_legacy(
            interaction,
            ctx,
            top_level="database",
            path_parts=["retention", "limits_show"],
            entries=[("Days", await ctx.retention.get_retention_days())],
            service_name="status",
        )

    @retention_group.command(name="limits_reset", description="Reset retention limits to defaults.")
    async def retention_limits_reset_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.retention.limits_reset", ctx):
            return
        ...
        await _send_legacy(
            ...
            ctx,
            top_level="database",
            path_parts=["retention", "limits_reset"],
    ...
    async def backfill_on_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.backfill.on", ctx):
            return
        await ctx.backfill.set_enabled(True)
        await _send_legacy(interaction, ctx, top_level="database", path_parts=["backfill", "on"], entries=[("Status", "enabled")], tone="success", service_name="status")
    ...
    async def backfill_off_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.backfill.off", ctx):
            return
        await ctx.backfill.set_enabled(False)
        await _send_legacy(interaction, ctx, top_level="database", path_parts=["backfill", "off"], entries=[("Status", "disabled")], tone="success", service_name="status")
    ...
    async def backfill_status_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.backfill.status", ctx):
            return
    ...
    @backfill_group.command(name="limits_set", description="Update backfill limits.")
    @app_commands.describe(days="Backfill window in days.")
    async def backfill_limits_set_command(interaction: discord.Interaction, days: int | None = None) -> None:
        if not await check_permission(interaction, "database.backfill.limits_set", ctx):
            return
        if days is None:
            await _send_legacy(
                interaction,
                ctx,
                top_level="database",
                path_parts=["backfill", "limits_set"],
                entries=[("Reason", "No changes provided")],
                tone="warning",
                sections=[("Next Step", [("Command", "/database backfill limits_show")])],
                service_name="status",
            )
            return
        if days <= 0:
    ...
    @backfill_group.command(name="limits_show", description="Show backfill limits.")
    async def backfill_limits_show_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.backfill.limits_show", ctx):
            return
        await _send_legacy(
            interaction,
            ctx,
            top_level="database",
            path_parts=["backfill", "limits_show"],
            entries=[("Days", await ctx.backfill.get_backfill_days())],
            service_name="status",
        )

    @backfill_group.command(name="limits_reset", description="Reset backfill limits to defaults.")
    async def backfill_limits_reset_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.backfill.limits_reset", ctx):
            return
        ...
        await _send_legacy(
            ...
            ctx,
            top_level="database",
            path_parts=["backfill", "limits_reset"],
            entries=[("Status", "reset"), ("Days", default_days)],
            tone="success",
            service_name="status",
        )
    ...
    async def backfill_run_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "database.backfill.run", ctx):
            return
...
def register_ai(ai_group: app_commands.Group, ctx: CommandContext) -> None:
    ...
    async def ai_on_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "ai.on", ctx):
            return
    ...
    async def ai_off_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "ai.off", ctx):
            return
    ...
    async def ai_status_command(interaction: discord.Interaction) -> None:
        if not await check_permission(interaction, "ai.status", ctx):
            return
    ...
    async def ai_model_set_command(interaction: discord.Interaction, task: app_commands.Choice[str], model: str) -> None:
        if not await check_permission(interaction, "ai.model_set", ctx):
            return
    ...
    async def ai_model_show_command(interaction: discord.Interaction, task: app_commands.Choice[str] | None = None) -> None:
        if not await check_permission(interaction, "ai.model_show", ctx):
            return
        status = ctx.ai.status()
        models = status.get("models", {}) if isinstance(status, dict) else {}
        await _send_ai_map(interaction, ctx, title="AI primary models", values=models if isinstance(models, dict) else {}, task=task.value if task else None)

    @ai_group.command(name="model_reset", description="Reset the AI model override for a task.")
    @app_commands.describe(task="AI task.")
    @app_commands.choices(task=AI_TASK_CHOICES)
    async def ai_model_reset_command(interaction: discord.Interaction, task: app_commands.Choice[str]) -> None:
        if not await check_permission(interaction, "ai.model_reset", ctx):
            return
        await ctx.ai.reset_model(task.value)
        await _send_legacy(interaction, ctx, top_level="ai", path_parts=["model_reset"], entries=[("Task", task.value), ("Status", "reset"), ("Scope", "primary")], tone="success", service_name="status")
    ...
    async def ai_fallback_set_command(interaction: discord.Interaction, task: app_commands.Choice[str], model: str) -> None:
        if not await check_permission(interaction, "ai.fallback_set", ctx):
            return
        if not _is_valid_provider_model(model):
    ...
    async def ai_fallback_show_command(interaction: discord.Interaction, task: app_commands.Choice[str] | None = None) -> None:
        if not await check_permission(interaction, "ai.fallback_show", ctx):
            return
        status = ctx.ai.status()
        models = status.get("fallback_models", {}) if isinstance(status, dict) else {}
        await _send_ai_map(interaction, ctx, title="AI fallback models", values=models if isinstance(models, dict) else {}, task=task.value if task else None)

    @ai_group.command(name="fallback_reset", description="Reset the AI fallback model override for a task.")
    @app_commands.describe(task="AI task.")
    @app_commands.choices(task=AI_TASK_CHOICES)
    async def ai_fallback_reset_command(interaction: discord.Interaction, task: app_commands.Choice[str]) -> None:
        if not await check_permission(interaction, "ai.fallback_reset", ctx):
            return
        await ctx.ai.reset_fallback_model(task.value)
        await _send_legacy(interaction, ctx, top_level="ai", path_parts=["fallback_reset"], entries=[("Task", task.value), ("Status", "reset"), ("Scope", "fallback")], tone="success", service_name="status")
    ...
    async def ai_run_command(interaction: discord.Interaction, task: app_commands.Choice[str], prompt: str, web: app_commands.Choice[str] | None = None) -> None:
        if not await check_permission(interaction, "ai.run", ctx):
            return
        await interaction.response.defer(ephemeral=True, thinking=True)
...
def register_admin(admin_group: app_commands.Group, ctx: CommandContext) -> None:
    """Legacy compatibility shim: /admin is no longer a canonical root."""
    _ = (admin_group, ctx)
