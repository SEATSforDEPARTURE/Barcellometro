...
sys.modules["app.core.service_registry"] = service_registry_stub
...
def test_sanitize_transcript_for_summary_replaces_offensive_terms() -> None:
    original = "Sei una stronza e rompi il cazzo, vaffanculo. Che merda."
    sanitized = mod._sanitize_transcript_for_summary(original)
...
def test_looks_like_summary_refusal_detects_refusal_phrases() -> None:
    ...
    assert mod._looks_like_summary_refusal(refusal)
    assert not mod._looks_like_summary_refusal(normal_summary)
...
def test_build_audio_note_summary_skips_refusal_output() -> None:
    async def _run() -> None:
        ai_service = SimpleNamespace(
            is_enabled=lambda: True,
...
def test_build_audio_note_summary_retries_after_refusal_and_returns_second_valid_summary() -> None:
    async def _run() -> None:
...
def test_build_audio_note_output_does_not_include_summary_section_when_summary_is_none() -> None:
    output = mod._build_audio_note_output(
        transcript_text="trascrizione originale",
        detected_lang="en",
        translation_text="traduzione italiana",
